...
#2 - 
#3 - 
#logging - 				logger.warning('Cancel Job - command from server for job_id:%s job_num:%s',str(job_id),str(job_num))
...
def getPrinterType():
	...
	if '23c1:d314' in split_data:
		...
		found = True
	#should explain this part to Greg
	#not sure what this does,
	#hey greg, nice to see you around, this shit right here parse lsusb for an un-named device (aka sketchy-ass repraps)
...
#DREW_DONE attempts connection to printer
def reconnectPrinter():
	global printer_profile, printer_firmware, printer_printerId, online
...
#DREW_DONE gets unique Ultimaker/Rerap ID
def get_unique_id():
...
def parseJSON(bodyString):
	...
	if job_cancel: #not getting past this if statement
...
def do_monitor():
	...
	if pron.p.printing:
		...
		if job_progress>=1.0:
			...
			spd = 500 #lower number = gets closer to full heating bar, but has bigger jump in the beginning
			trgt = 229 #won't shoing "heating" > 230C
			printer_tool1_temp = int((-spd/(temp_curve+(spd/trgt)))+trgt)#fake it untill you make it ;)
			#^function to show time with limit of 229 (never break the GUI)
