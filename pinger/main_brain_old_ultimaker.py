...
#git update
	...
	if printer_printerId == '': # no printer
		...
		if printer_type == 'Makerbot':
			print 'found a makerbot, switch to the makerbot script'

			#DREW WITE SHIT
			arg = ['sudo /home/pi/raspi/piConfig/startup_makerbot.sh']
			# call subprocess
		...
		elif printer_type == 'LulzBot AO100':
...
def parseJSON(bodyString):
	...
	job_cancel = False
#cancel if...
#1- job_id match and there's a cancel_cmd
#2- job_id's don't match and new id isn't blank
#3- no job_id key and printer is doing something
	...
	#1
				print "canceling from CANCEL_CMD"
				job_cancel = True
		else:
		#new job_id don't match or
			...
			if job_id != '': # not nitial value
				#delete current job
	#2
				print "canceling from JOBS_NOT_MATCHING"
				job_cancel = True
		#Cancel job (currentJob or previous job that was left running)
	else: #no job_id and printer is printing something
		if printer_inUse:
	#3
			print "canceling from PRINTING_BUT_"
			job_cancel = True
